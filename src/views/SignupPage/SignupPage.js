...
class Signup extends React.Component {
  ...
  constructor(props) { 
    ...
    this.state = { 
      ...
      email: '', email_check: false, 
      name: '', name_check: false, 
			password: '', password_check: false, 
      password_confirmed: '', password_confirmed_check: false, 
  ...
  goLoginPage = () => { this.props.history.push('/signin'); };
  ...
  handlePasswordConfirmed = e => { 
    ...
    if (password_confirmed !== prev_password) { 
      this.setState({password_confirmed_check: false, password_confirmed: password_confirmed});
      return; 
    }
    this.setState({password_confirmed_check: true, password_confirmed: password_confirmed});
  }
  ...
  submitSignUp = () => {
    const { 
      email, name, password, 
      agree_clause_essential, agree_collect_personalinfo_essential,
    } = this.state; 
  ...
  render() { 
    ...
    const { 
      ...
      password_confirmed, password_confirmed_check, 
    } = this.state; 
    ...
    if (password_check) { password_check_msg = <span className="success-msg">사용가능한 비밀번호입니다.</span>;}
    else if (0 < password.length && password.length < 6) { password_check_msg = <span className="warning-msg">6자리 이상 입력해 주세요</span>;}
